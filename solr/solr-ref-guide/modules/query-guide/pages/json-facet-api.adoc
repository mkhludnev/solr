...
You can develop your own aggregation as ValueSourceParser (see ..)

[source,java,indent=0]
----
include::example$CustomAggTest.java[tag=custom-agg-impl]
----

deploy jar as a SolrModule (see ..) and enable in a certain collection via Config API (see ..) or just configure it via solrconfig.xml. Mind `agg_` prefix.
...
"name": "agg_custom",
"class": "org.pkg...CustomFacet",
}
}' http://localhost:8983/api/collections/techproducts/config
----

Then one can invoke custom aggregation via JSON Facet API.

[source,java,indent=0]
----
include::example$CustomAggTest.java[tag=custom-agg-api-req]
----
